defmodule ExOneroster.Web.OrgController do
  ...
  def index(conn, _params) do
    orgs = Orginizations.list_orgs()
    render(conn, "index.json", orgs: orgs)
  end

  def create(conn, %{"org" => org_params}) do
    with {:ok, %Org{} = org} <- Orginizations.create_org(org_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "org" => org_params}) do
    ...
    with {:ok, %Org{} = org} <- Orginizations.update_org(org, org_params) do
      render(conn, "show.json", org: org)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    org = Orginizations.get_org!(id)
    with {:ok, %Org{}} <- Orginizations.delete_org(org) do
