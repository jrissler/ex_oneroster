defmodule ExOneroster.OrganizationsTest do
  ...
  describe "orgs" do
    ...
    test "create_org/1 with valid data creates a org" do
      ...
      assert {:ok, %Org{} = org} = Organizations.create_org(params_for(:org, dateLastModified: org_params.dateLastModified))
    ...
    test "update_org/2 with valid data updates the org" do
      ...
      assert {:ok, org} = Organizations.update_org(existing_org, params_for(:org, sourcedId: "Bond..James Bond", dateLastModified: existing_org.dateLastModified))
      assert %Org{} = org
